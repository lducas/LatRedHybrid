...
def find_r_GLP_over(n, q, k_vals = [6], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	m = 2*n + 1
	det = q**(n)
	...
	for r in k_vals:
		...
		def f(x):
			f_x = RR(bkzcosts_mult_rounds(dim,x) - log((nr_loops_new(r,c,dim, det, x, error_norm,q, size_S)*rt_NP_over(dim)), 2))
			return f_x
		
		delta_r = find_zero(f, "x", min_delta, max_delta, value=0, args = {}, eps = 1, print_steps = false)
		bit_hardness_r = RR(bkzcosts_mult_rounds(dim,delta_r) + 1 - log(pr_term * prob_NP(dim, det, error_norm, delta_r,q), 2))
