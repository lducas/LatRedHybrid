...
def find_r_NTRU_product_form(n, q, norm_f_square, d_g, k_vals = [6], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	m = 2*n
	det = q**(n)
...
#			print "p_succ done"
			size_S = 1
			f_x = RR(bkzcosts(dim,x) - log((nr_loops_NTRU(r,c_1,c_minus_1,size_S,dim, det, x, error_norm,q)*2**15) / (p_succ), 2))
#			print "f_x done"
			return f_x
