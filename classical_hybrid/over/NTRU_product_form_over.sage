...
def find_r_NTRU_product_form_over(n, q, norm_f_square, d_g, k_vals = [6], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	m = 2*n
	det = q**(n)
	...
	for r in k_vals:
		...
		def f(x):
			size_S = 1
			f_x = RR(bkzcosts_mult_rounds(dim,x) - log((nr_loops_NTRU(r,c_1,c_minus_1,size_S,dim, det, x, error_norm,q)*rt_NP_over(dim)), 2))
			return f_x
		
		delta_r = find_zero(f, "x", min_delta, max_delta, value=0, args = {}, eps = 1, print_steps = false)
		p_S = prob_S_NTRU(pr_term, dim, det, error_norm, delta_r,q)
		p_succ = p_S
