...
def find_r_BLISS_under(n, q, d_1, d_2, k_vals = [6], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	m = 2*n
	det = q**(n)
	...
	for r in k_vals:
		...
		def f(x):
			...
			p_succ = p_S
			size_S = 1
			f_x = RR(bkzcosts_one_round(dim,x) - log((nr_loops_BLISS(r,c_2,c_4,size_S,dim, det, x, error_norm,q)*rt_NP_under(dim)) / (p_succ), 2))
			return f_x
