...
def find_r_NTRUprime_under(p, q, t, k_vals = [6], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	n = p
	m = 2*n
	...
	for r in k_vals:
		...
		def f(x):
			...
			size_S = 2 + 2 * (n - t) * p_S
			f_x = RR(bkzcosts_one_round(dim,x) - log((nr_loops_NTRUprime(r,c_1,c_1,size_S,dim, det, x, error_norm,q)*rt_NP_under(dim)) / (p_succ), 2))
			return f_x
